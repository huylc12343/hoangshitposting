  ...
  export default function Cart_HeroSection() {
    ...
    const [shippingMethod, setShippingMethod] = useState("viettel_post");
    ...
    /** Cart info end */
    const totalItemCount = items ? items.reduce((sum, item) => sum + item.amount, 0) : 0;
    ...
    const handleOrderSubmit = async () => {
      ...
      if (holdId) {
        ...
        MerchService.buyMerch({
          ...
          .catch((error) => {
            alert(
              "Có lỗi xảy ra trong quá trình đặt mua merch. Vui lòng thử lại sau."
            );
            setLoading(false);
            navigate("/");
          });
        ...
        <>
          ...
          >
            ...
            <div className="relative z-10 w-full h-full flex flex-col justify-center items-center max-w-7xl text-white mt-24">
              <img src={empty_img} className=" font-bold mt-14 w-[250px] h-[250px]"/>
              <h1 className="text-2xl lg:text-5xl font-imbue"
              style={{weight:600}}>
                Giỏ hàng của bạn hiện đang trống...
              </h1>
              <p className="font-Averta-CY text-base mt-8 mb-0">Hãy cùng chúng mình bắt đầu chuyến phiêu lưu vào thế giới cá hồi hoang ngay nhé!"</p>
              <button
    ...
    const content = firstName + " " + lastName + " " + "Thanh toan Merch";
    const amount =
